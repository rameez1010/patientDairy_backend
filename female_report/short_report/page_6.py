...
def page_6(results: List[BioMarker]):
    # c = canvas.Canvas("female_report/short_report/page_6.pdf", pagesize=letter)
    buffer = io.BytesIO()
    ...
    if vitaminb12 is not None:
        vitaminb12_bar = convert_png_to_white_bg("female_report/short_report/ANAO_bar.png")
        ...
        c.drawString(295, y_position + 35, "138.0")
        ...
        if vitaminb12.value < 138.0:
            c.circle(270, y_position + 17, 11, fill=1)
        elif 138.0 <= vitaminb12.value <= 652.0:
            c.circle(347, y_position + 17, 11, fill=1)
        elif 652.0 < vitaminb12.value <= 800.0:
    ...
    if vitamind is not None:
        vitamind_bar = convert_png_to_white_bg("female_report/short_report/ANOA_bar.png")
        ...
        if vitamind.value < 76.0:
            c.circle(270, y_position + 17, 11, fill=1)
        elif 76.0 <= vitamind.value < 200.0:
            c.circle(345, y_position + 17, 11, fill=1)
        elif 200.0 <= vitamind.value <= 250.0:
    ...
    if vitamina is not None:
        vitamina_bar = convert_png_to_white_bg("female_report/short_report/ANONA_bar.png")
