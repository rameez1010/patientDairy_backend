...
def page_1(results: List[BioMarker]):
    print("Page 1 results are:", results)
    print("type of Page 1 results are:", type(results))
    # c = canvas.Canvas("male_report/short_report/page_1.pdf", pagesize=letter)
    buffer = io.BytesIO()
...
assess the risk of cardiovascular diseases and guide treatment to manage lipid imbalances.
    ...
    if cholesterol is not None:
        cholesterol_bar = convert_png_to_white_bg(
            "male_report/short_report/NOA_bar.png"
        )
        c.drawImage(
            cholesterol_bar,
            70,
        ...
        c.drawString(433, y_position + 35, "5.2")
        ...
        if cholesterol.value < 3.5:
            c.circle(280, y_position + 17, 11, fill=1)
        elif 3.5 <= cholesterol.value <= 5.2:
    ...
    if ldl_cholesterol is not None:
        ldl_bar = convert_png_to_white_bg("male_report/short_report/OA_bar.png")
        c.drawImage(
    ...
    if hdl_cholesterol is not None:
        hdl_bar = convert_png_to_white_bg("male_report/short_report/ANO_bar.png")
        c.drawImage(
    ...
    if non_hdl is not None:
        nonhdl_bar = convert_png_to_white_bg("male_report/short_report/ONA_bar.png")
        c.drawImage(
    ...
    if triglycerides is not None:
        triglycerides_bar = convert_png_to_white_bg(
            "male_report/short_report/NONA_bar.png"
        )
        c.drawImage(
            triglycerides_bar,
            70,
        ...
        c.setFillColor("gray")
        c.setFont("Roboto-Regular", 9)
        c.drawString(300, y_position + 35, "0.4")
        c.drawString(380, y_position + 35, "0.99")
        c.drawString(460, y_position + 35, "1.7")
        ...
        if triglycerides.value < 0.4:
            c.circle(270, y_position + 17, 11, fill=1)
        elif 0.4 <= triglycerides.value <= 0.99:
            c.circle(345, y_position + 17, 11, fill=1)
        elif 0.99 < triglycerides.value <= 1.7:
    ...
    if choHDL is not None:
        cholHDL_bar = convert_png_to_white_bg("male_report/short_report/ONA_bar.png")
        c.drawImage(
