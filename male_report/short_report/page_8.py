...
def page_8(results: List[BioMarker]):
    # c = canvas.Canvas("male_report/short_report/page_8.pdf", pagesize=letter)
    buffer = io.BytesIO()
    ...
    if alp is not None:
        alp_bar = convert_png_to_white_bg("male_report/short_report/ANONA_bar.png")
        ...
        c.drawString(280, y_position + 35, "40.0")
        c.drawString(346, y_position + 35, "45.0")
        c.drawString(412, y_position + 35, "100.0")
        c.drawString(476, y_position + 35, "129.0")
    ...
    if alt is not None:
        alt_bar = convert_png_to_white_bg("male_report/short_report/OA_bar.png")
        ...
        c.setFillColor("gray")
        c.setFont("Roboto-Regular", 9)
        c.drawString(375, y_position + 35, "50.0")
    ...
    if ast is not None:
        ast_bar = convert_png_to_white_bg("male_report/short_report/NONA_bar.png")
        ...
        c.drawString(380, y_position + 35, "30.0")
        c.drawString(460, y_position + 35, "35.0")
        ...
        elif 10 <= ast.value <= 30:
            c.circle(347, y_position + 17, 11, fill=1)
        elif 30 < ast.value <= 35:
    ...
    if ggt is not None:
        ggt_bar = convert_png_to_white_bg("male_report/short_report/AONA_bar.png")
        ...
        c.drawString(380, y_position + 35, "50.00")
        c.drawString(460, y_position + 35, "62.00")
        ...
        if ggt.value < 10:
            c.circle(270, y_position + 17, 11, fill=1)
        elif 10 <= ggt.value <= 50:
            c.circle(347, y_position + 17, 11, fill=1)
        elif 50 < ggt.value <= 62:
            c.circle(430, y_position + 17, 11, fill=1)
        else:
            c.circle(510, y_position + 17, 11, fill=1)
        y_position -= 53
    ...
    if bilirubin is not None:
        bilirubin_bar = convert_png_to_white_bg("male_report/short_report/NONA_bar.png")
    ...
    if albumin is not None:
        albumin_bar = convert_png_to_white_bg("male_report/short_report/ANOA_bar.png")
